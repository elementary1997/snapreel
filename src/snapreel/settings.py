...
Здесь нет ни Qt, ни побочных эффектов — только описание формы и разбор
...
GROUPS: tuple[Group, ...] = (
    ...
    Group(
        ...
        (
            Field(
                "min_seconds",
                "Минимальная длительность, с",
                "float",
                "раньше этого остановить запись нельзя",
            ),
            Field(
                "max_seconds",
                "Максимальная длительность, с",
                "float",
                "по достижении запись закроется сама",
            ),
            Field("fps", "Кадров в секунду", "int", "1..120"),
            Field("capture_cursor", "Рисовать курсор", "bool"),
            Field("capture_audio", "Писать звук", "bool"),
            Field("audio_device", "Устройство звука", "audio", "пусто — звук не пишется"),
            Field(
                "screen_index",
                "Номер экрана",
                "int",
                "macOS: с какого экрана писать, -1 — определить самому",
            ),
        ),
    ),
    Group(
    ...
    Group(
        ...
        (
            ...
            Field(
                ...
                "Максимальная ширина",
                "int",
                "GIF только уменьшается, 0 — не менять",
            ),
        ),
    ),
    Group(
        ...
        (
            ...
            Field("keep_days", "Срок хранения, дней", "int", "0 — не удалять"),
        ),
    ),
    Group(
        "Прочее",
        (
            ...
            Field(
                ...
                "choice",
                choices=THEMES,
                labels=(("auto", "Как в системе"), ("dark", "Тёмная"), ("light", "Светлая")),
            ),
...
# события: маска у каждой оконной системы своя, а имена клавиш приводит к
# общему виду окно (`settings_ui.keysym`).
