...
def powershell(script: str, timeout: float = 60) -> subprocess.CompletedProcess:
    ...
    превращается в «?». Через это проходит всё русское, что мы отдаём
    Windows: текст уведомлений, описание ярлыка, пути с чужими буквами.
    Нашлось приёмкой на раннере с английской системой; на русской всё
    работало.
