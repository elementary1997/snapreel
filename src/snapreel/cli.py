...
def _update(args) -> int:
    ...
    # работает всё ещё прежняя версия: она уже поднята, а новая только легла
    # на диск. Из трея это делается пунктом меню, отсюда — руками
    print("перезапустите snapreel, чтобы новая версия заработала")
    return 0
...
def _doctor(cfg) -> int:
    ...
    # на комбинации по-настоящему и сразу отпускаем. Где так делать нельзя
    # (macOS: проба без разрешения убила бы процесс) — так и говорим, а не
    # выдаём перечень известных причин за проверку
    from . import hotkeys

    silent = hotkeys.probe(cfg, env)
    if silent:
        print("хоткеи:         не слушаются")
        print(f"                {silent}")
        problems.append(f"горячие клавиши не слушаются: {silent}")
    elif hotkeys.can_probe(env):
